#' heatmap2(x,y,z,filter,resolution,zlabel,outfile)
...
heatmap2<- function(x,y,z,filter,resolution,zlabel,outfile,nlevels,
  ...
  if (missing(places) & missing(route)) {
    contour(map1,nlevels=nlevels,plot.title={
    ...
    contour(map1,nlevels=nlevels,plot.title={title(xlab="Longitude",cex.lab=1)
                     mtext("Latitude",2,cex=1,line=3,las=0)
                     mtext(zlabel,4,cex=1,line=0.8,las=0)},
