...
#shapefile <- "/Users/larsnesheim/Downloads/speed1.kml"
speed      <- parms$speed
...
\title{Region:   \Sexpr{regnames$name[region_id]} \\
       ...
       Impact on commercial property values of \Sexpr{infrastructure_type} improvement project near \Sexpr{project_location} }
...
To predict commercial rateable values and  new travel times for the region, this report assumes:
\begin{itemize}
\item Rateable values are determined by 1) commercial use type (e.g. offices, shops, warehouse, etc.) and size and 2) location. Details of the data used to estimate the model are presented in Section \ref{section:data}. Model details are presented in Section \ref{section:model}.
...
stargazer(m1,keep=keeplist,no.space=TRUE,single.row=TRUE,
          title="Dependence of log ratable values on structure")
@
