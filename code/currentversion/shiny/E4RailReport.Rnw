...
rail<-readOGR(shapefile,layer=layer1)
if (class(rail)=="SpatialPointsDataFrame") {
  rail1<-Line(coordinates(rail)[,1:2])
} else if (class(rail)=="SpatialLinesDataFrame") {
  rail1<-rail@lines[[1]]@Lines[[1]]
}
raillength<-LineLength(rail1,longlat=TRUE)/1.6
njunctions<-nrow(rail1@coords)
...
junctions<-knn(newdata$m2data_old[,c("longitude","latitude")],coordinates(rail1)[,1:2],
...
\textbf{\Sexpr{regnames$name[region_id]}} region. Figure \ref{fig:MAP} displays a map of the new \Sexpr{infrastructure_type}. The rail line is approximately \Sexpr{round(raillength,1)} miles long and has \Sexpr{njunctions} junctions. It begins near \Sexpr{start} and finishes near \Sexpr{finish}.

% FIG 1:  MAP OF REGIONS
<<MAP,fig.cap="Route map of rail",fig.align="center",fig.pos="H">>=
if (region_id!=1) {
  plot(regions[region_map_index[region_id],],main="Route map for new rail")
} else if (region_id==1) {
  plot(regions[region_map_index[c(3,4,8)],],main="Route map for new rail")
}
lines(coordinates(rail1),col="red")
points(coordinates(rail1)[,1:2],col="red",pch=2)
@   
...
title<-c("Public transport travel times (minutes): pre investment",
         "Public transport times (minutes): post investment",
         "Change in public transport travel time (minutes): (new minus old)")
...
<<trans_TIME_PLOTS_1,fig.cap="Driving time">>=
  ...
  for (i1 in 1:nfig) {
    title=paste0("Public transport time reduction: ",town[i1])
    plot(density(deltatranstime[,i1],na.rm=TRUE),main=title,xlab="minutes of travel time")
...
#             route=as.data.frame(rail)[,c(1,3,4)],routelabel = "new rail")
  }
@
...
<<trans_TIME_PLOTS_2,fig.cap="Driving time">>=
  if (ncol(deltatranstime)>3) {
    for (i1 in 4:ncol(deltatranstime)) {
      title=paste0("Public transport travel time reduction: ",town[i1])
      plot(density(deltatranstime[,i1],na.rm=TRUE),main=title,xlab=" minutes of travel time")
    }
  }
#    heatmap1(newdata$m2data_new$longitude,newdata$m2data_new$latitude,deltatranstime[,i1],
