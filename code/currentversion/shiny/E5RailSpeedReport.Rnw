...
infrastructure_type <- "rail"   # c("rail","road","settlement")
datastub<-"nondom"
...
method     <- parms$method  # c("newroute","newspeed")
shapefile  <- parms$speedfile
#shapefile  <- "/Users/larsnesheim/Downloads/region11_speed1.kml"
speed      <- parms$speed
multiplier <- 1+parms$multiplier/100
...
newdata<-A5CreateTransport(region_id,infrastructure_type=infrastructure_type,
                           method = method,shapefile,multiplier,speed,depvar,dirs,usemapdir=FALSE,
...
if (is.null(shapefile)) {
  if (region_id!=1) {
    m_sq<-gArea(spTransform(regions[region_map_index[region_id],],ukgrid))
  } else if (region_id==1) {
    m_sq<-gArea(spTransform(regions[region_map_index[c(3,4,8)],],ukgrid))
  }
  km_sq <- m_sq/1e6
  project_location<-as.character(regnames$name[region_id])
} else {
  # Load mapfile
  layer1       <- ogrListLayers(shapefile)
  speedregion <- readOGR(shapefile,layer=layer1)
  
  project_centroid<-gCentroid(speedregion) 
    
  project_location<-knn(newdata$m2data_old[,c("longitude","latitude")],
                        coordinates(project_centroid),
                        newdata$m2data_old$postal_town,k=1)
  project_location<-as.character(project_location)
 project_centroid<-coordinates(project_centroid)

  # Land area of settlement
  m_sq  <- gArea(spTransform(speedregion,ukgrid))
  km_sq <- m_sq/1e6
}
...
i1<-grep("\\<trans_",colnames(newdata$m2data_old))
i2<-grep("\\<trans_",colnames(newdata$m2data_new))

transtime1<-newdata$m2data_old[,i1]
transtime2<-newdata$m2data_new[,i2]
deltatranstime <- transtime2-transtime1
a1<-matrix(unlist(strsplit(colnames(transtime1),"_")),2,length(transtime1))[2,]
#a1[ndest+1]<-"RAIL"
names(transtime1)<-a1
names(transtime2)<-a1
names(deltatranstime)<-a1
...
\title{Region:   \Sexpr{regnames$name[region_id]} \\
       ...
       Impact on rateable values of \Sexpr{infrastructure_type} improvement project near \Sexpr{project_location} }
...
The project is a \textbf{\Sexpr{infrastructure_type}} transport improvement project in the 
\textbf{\Sexpr{regnames$name[region_id]}} region. Figure \ref{fig:MAP} displays a map of the project area.
The project area is approximately \Sexpr{round(km_sq,2)} square kilometres. It is located near
\Sexpr{project_location}. The project aims to reduce travel times to all destinations to \Sexpr{round(100*multiplier,1)}
\% of baseline travel times.

% FIG 1:  MAP OF REGIONS
<<MAP,fig.cap="Map of travel improvement area",fig.align="center",fig.pos="H">>=
# TODO: use ggplot to plot these polygons better
if (region_id!=1) {
  plot(regions[region_map_index[region_id],],main="Map of travel improvement area")
} else if (region_id==1) {
  plot(regions[region_map_index[c(3,4,8)],],main="Map of travel improvement area")
}
if (!is.null(shapefile)) {
  lines(coordinates(speedregion@polygons[[1]]@Polygons[[1]]),col="red")
}
@   

To predict rateable values and new travel times for the region, this report assumes:
\begin{itemize}
\item Rateable values are determined by 1) property size and commercial use type and 2) location. Details of the data used to estimate the model are presented in Section \ref{section:data}. Model details are presented in Section \ref{section:model}.
...
\item Travel times to \Sexpr{nrow(dest)} cities are the dominant factors for property values in this region. These cities are: (\textbf{ \Sexpr{town}} ).
\item New travel times after the investment equal \Sexpr{multiplier} multiplied by the baseline travel time 
...
\item The model provides the best approximation to the short run impact of the \Sexpr{infrastructure_type} improvement
project.
\end{itemize}
...
<<trans_TIME_SUMSTATS,results='asis',CACHE=FALSE,echo=FALSE>>=
...
ntable<-ceiling(ncol(transtime1)/6)
n1    <-ceiling(ncol(transtime1)/ntable)
for (i1 in 1:ntable) {
  i2 <- n1*(i1-1)+(1:6)
  i2 <- i2[i2<=ncol(transtime1)]
  stargazer(transtime1[,i2],
            transtime2[,i2],
            deltatranstime[,i2],title=title,align=TRUE, out.header=TRUE,digits=1,
...
\item VOA data rateable value data.
...
<<trans_TIME_PLOTS_1,fig.cap="Driving time">>=
  nfig<-min(3,ncol(deltatranstime))
  for (i1 in 1:nfig) {
    title=paste0("Transit time reduction: ",town[i1])
    plot(density(deltatranstime[,i1],na.rm=TRUE),main=title,xlab="minutes of transit time")
#    heatmap1(newdata$m2data_new$longitude,newdata$m2data_new$latitude,deltatranstime[,i1],
#             resolution=0.01,
#             zlabel="$\\Delta$ travel time (minutes)",
#             outfile=paste0(dirs$outdir,"/deltatranstime_",dest$shortname[i1],".pdf"),
#             places=dest[,c("shortname","longitude","latitude")])
  }
@

% FIGURE 2.2: Densities and heatmaps of travel times
<<trans_TIME_PLOTS_2,fig.cap="Driving time">>=
  if (ncol(deltatranstime)>3) {
    for (i1 in 4:ncol(deltatranstime)) {
      title=paste0("Transit time reduction: ",town[i1])
      plot(density(deltatranstime[,i1],na.rm=TRUE),main=title,xlab=" minutes of transit time")
 #     heatmap1(newdata$m2data_new$longitude,newdata$m2data_new$latitude,deltatranstime[,i1],
#               resolution=0.01,
