...
dirs         <- B2SetPath(RootDir,CodeDir,DataRoot,region_id,datastub=datastub)
...
newdata<-A4CreateSettlement(region_id,dirs,m2data_new = newdata,depvar=depvar,
...
\title{Region:   \Sexpr{regnames$name[region_id]} \\
       ...
       Impact on residential properties of new \Sexpr{infrastructure_type} near \Sexpr{project_location} }
...
and \Sexpr{format(nbusinesses,big.mark=",")} near \Sexpr{project_location}. Tables \ref{table:settlement} and \ref{table:settlement sumstats} 
...
To predict new residential property values, this report assumes:
\begin{itemize}
\item Residential property values are determined by 1) property size, 2) property type, and 3) location. Details of the data used to estimate the model are presented in Section \ref{section:data}. Model details are presented in Section \ref{section:model}.
...
stargazer(data.frame(newprice,deltaprice_lo,deltaprice_med,deltaprice_hi),
          ...
          notes="This table shows the distribution of predicted residential property values 
...
<<PRICE_PLOTS_3,fig.cap="Impact of project on prices">>=
  title="Residential prices before the investment"
...
<<PRICE_PLOTS_6,fig.cap="Impact of project on prices">>=
  ...
  xres <- (max(x)-min(x))/20
  yres <- (max(y)-min(y))/20
  zlabel<-"change in price (2017 GBP) "
  map1 <- interp(x=x,
                 y=y,
                 z=deltaprice[iz],
                 yo=seq(min(y),max(y),by=xres),
