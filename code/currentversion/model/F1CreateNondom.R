#' Create base dataframe for new nondom properties, based on user inputs
#' @param nproperty     Number of new properties to build
...
#' load("m2data.RData")
#' newdata <- C1CreateSettlement(nproperty,popdensity,mapfile,model_laname,m2data)

F1CreateNondom<-function(nproperty,popdensity,mapfile,model_laname,m2data) {
  ...
  # Replace c("usecode","total_floor_area") with random draws from useCodeData
  set.seed(345)
  load(file=paste0(dirs$rootdir,"/data/nondom/usecodedata.RData"))
  isample<-sample(c(1:nrow(useCodeData)),size=nrow(modeldata),replace=TRUE)
  modeldata[,c("usecode","total_floor_area")] <- useCodeData[isample,c("usecode","total_floor_area")]
  
  # 1)  Create (longitude,latitude) using spsample(...)
  #     type = "regular"  regularly spaced points
  newdata <- data.frame(coordinates(spsample(settlement,round(1.2*nproperty,0),type="regular")))
  if (nrow(newdata)>nproperty) {
    newdata<-newdata[1:nproperty,]
  } else if (nrow(newdata)<nproperty) {
    nproperty = nrow(newdata)  
  }
  names(newdata) <- c("longitude","latitude")  
  
  set.seed(436532)
  isample<-sample(c(1:nrow(modeldata)),size = nproperty,replace=TRUE)

  # 2) (propertytype,tenure,total_floor_area)
  varlist <- c("usecode","total_floor_area")
