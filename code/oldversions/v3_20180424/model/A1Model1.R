...
if (!file.exists(DestinationFile)) {
 ...
 if (plotflag>0) {
   ...
   map0 <- interp(x=m1data$longitude, y=m1data$latitude, z=(splinehat1-mean(splinehat1))/sd(splinehat1), 
...
if (plotflag>0) {
  tempfile<-paste0(dirs$outdir,'/density_location.eps')
  if (plotflag==2) postscript(tempfile)
...
#  resolution2<-10000
#  map2 <- interp(x=easting, y=northing, z=std_lv, 
#                 yo=seq(min(northing),max(northing),by=resolution2), 
#                 xo=seq(min(easting),max(easting),by=resolution2), duplicate="mean")
#  
  if (plotflag==2) {
    postscript(file=paste0(dirs$outdir,'/model1_location.eps'))
  }
...
#  filled.contour(map2, color.palette=rainbow, 
#                 plot.title={
#                   title(xlab="Easting",cex.lab=1)
#                   mtext("Northing",2,cex=1,line=3,las=0)
#                   mtext("Standardized location value",4,cex=1,line=0.8,las=0)
#                 }, plot.axes={points(dest[dest$shortlist,5:6], pch=24); axis(1); axis(2); 
#                   text(dest[dest$shortlist,5:6], pos=1, labels=dest[dest$shortlist,2], cex=0.7);
#                 },nlevels=30)
  
   if (plotflag==1) {
     dev.copy2eps(file=paste0(dirs$outdir,'/model1_location.eps'))
     dev.copy2pdf(file=paste0(dirs$outdir,'/model1_location.pdf'))
   }
   dev.off()
   
   # plot logprice0 vs size and price0 vs size
   tempfile<-paste0(dirs$outdir,"/model1_size_logp.eps")
   if (plotflag==2) postscript(tempfile)
   plot(m1data$total_floor_area,logprice1-min(logprice1),pch=".",
   ...
   p0    <- mean(m1data$pricepaid[i1])
   tempfile<-paste0(dirs$outdir,"/model1_size.eps")
   if (plotflag==2) postscript(tempfile)
   x<-plot(m1data$total_floor_area,exp(logprice1-min(logprice1))*p0/1000,pch=".",
