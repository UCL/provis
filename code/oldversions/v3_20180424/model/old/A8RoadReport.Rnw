...
if (infrastructure_type=="road") {
  longroutefile<-paste0(RootDir,"/code/shiny/roads/",routefile)
  layer1<-ogrListLayers(longroutefile)
  road<-readOGR(longroutefile,layer=layer1)
  ...
  for (i1 in 1:njunctions) {
    for (i2 in 1:50) {
      junctions[[i1]]<-revgeocode(coordinates(road1)[i1,1:2],source="google",output="more") 
