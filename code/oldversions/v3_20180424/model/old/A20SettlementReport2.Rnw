...
#                     "model1_all","model2_new")
 ...
 # library(broom)
  ...
  load(paste0(RootDir,"/data/RICS/farms3.RData"))
  farms3<-data.frame(farms3$longitude,farms3$latitude,farms3$price_per_acre)
  farms3<-farms3[complete.cases(farms3), ]
  colnames(farms3)<-c("long","lat","price_per_acre")
  rics_dta<-SpatialPoints(farms3[,1:2])

  farm.coords <- tidy(rics_dta)[c("long", "lat")]
  set.coords <- tidy(houses_convexhull)[c("long", "lat")]
  nn <- get.knnx(farm.coords, set.coords, k=1)
  rics_price<-1e6*farms3[nn$nn.index[which(nn$nn.dist==min(nn$nn.dist))],3]/sqm_per_acre
  farmprice_rics  <- rep(m_sq*rics_price/nhouses,length.out=length(newprice))
  deltaprice_rics<-newprice-farmprice_rics
...
stargazer(data.frame(newprice,deltaprice_lo,deltaprice_hi,deltaprice_hi, deltaprice_rics),
