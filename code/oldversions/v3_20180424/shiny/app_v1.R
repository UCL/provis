...
server <- function(input, output) {
  ...
  observeEvent(input$run_settlement,{
    ...
    #dirs<-B2SetPath(RootDir,region$id)
    #browser()
    #workdir<-getwd()
    #setwd(paste0(CodeDir,"/shiny/www"))
    ...
    outfile$report<-paste0(texfile,".pdf")
    setwd(workdir)
    page_id$n<-3
  })
  ...
  observeEvent(input$run_transport,{
    ...
    if (input$method=="newroad") {
      knit2pdf(input = paste0(CodeDir,"/shiny/D2RoadReport.Rnw"),
               output = paste0(CodeDir,"/shiny/www/",
                              "region",region$id,"_",
                              "road",project$id,".tex"),
               clean = TRUE,envir=globalenv())
      outfile$report<-paste0("region",region$id,"_","road",project$id,".pdf")
    } else if (input$method=="newspeed") {
      knit2pdf(input = paste0(CodeDir,"/shiny/D3RoadSpeedReport.Rnw"),
               output = paste0(CodeDir,"/shiny/www/",
                               "region",region$id,"_",
                               "roadspeed",project$id,".tex"),
               clean = TRUE,envir=globalenv())
      outfile$report<-paste0("region",region$id,"_","roadspeed",project$id,".pdf")
    }
...
#    filename = paste0("region",region$id,"_settlement1.pdf"),
#    content = function(file) {
#      file.rename(out,file)
##      out = knit2pdf("D1SettlementReport.Rnw",clean=TRUE)
#    },contentType="application/pdf"
#  )
#  output$report = downloadHandler(
#    filename = paste0("region",region$id,"_settlement1.pdf"),
#    content = function(file) file.copy(paste0(CodeDir,"/shiny/www/region",
#                                              region$id,"_settlement1.pdf"), file, overwrite = TRUE),
#    contentType = 'application/pdf'
#  )
