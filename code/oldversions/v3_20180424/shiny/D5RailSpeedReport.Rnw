...
#    for (i1 in 1:50) {
#    project_location_full<-try(revgeocode(coordinates(project_centroid),output="more"))
...
#  } else {
#    project_location<- "Travel project"
#  }
...
i2<-grep("\\<trans_",colnames(newdata$m2data_old))
...
stargazer(transtime1,
          transtime2,
          deltatranstime,title=title,align=TRUE, out.header=TRUE,digits=1,
...
<<MODEL2_SUMSTATS,results='asis',fig.pos="H">>=
  ...
  if (m2flag==0) {
    ...
    load(paste0(dirs$outdir,"/m2ols5.RData"))
    m2<-m2ols5
  }
