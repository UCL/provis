A3Model2_vlist1<-function(region=2,datastub="m11") {
  spec<-"linear"
  if (spec=="linear") {
    if (region!=1 & region!=5) {
      ...
      vlist1<-paste0("location_value~builtuparea_pct+restrictedland_pct+localplanrate+greenbelt",
                     ...
                     "+I(avgtime_station>=30)",
                     "+I((avgtime_station<30)*log(1+avgtime_station))",
                     "+I((avgtime_town>=30))",
                     "+I((avgtime_town<30)*log(1+avgtime_town))")
      } else if (datastub=="nondom") {
        vlist1<-paste0("location_value~builtuparea_pct+restrictedland_pct+localplanrate+greenbelt",
                       "+lu_domestic_shr+lu_gardens_shr+lu_nondom_shr+lu_road_shr+lu_rail_shr",
                       "+lu_greenspace_shr+lu_water_shr+popdensityOA+imddecile+prob_4band",
                       "+I(distance_airport>=10)",
                       "+I((distance_airport<10)*log(distance_airport))",
                       "+I(distance_motorway>=10)",
                       "+I((distance_motorway<10)*log(distance_motorway))",
      ...
      if (datastub=="m11") {
        vlist1<-paste0("location_value~builtuparea_pct+restrictedland_pct+localplanrate+greenbelt",
                       ...
                       "+I((avgtime_town>=21))",
                       "+I((avgtime_town<21)*log(1+avgtime_town))")
      } else if (datastub=="nondom") {
        vlist1<-paste0("location_value~builtuparea_pct+restrictedland_pct+localplanrate+greenbelt",
                       "+lu_domestic_shr+lu_gardens_shr+lu_nondom_shr+lu_road_shr+lu_rail_shr",
                       "+lu_greenspace_shr+lu_water_shr+popdensityOA+imddecile+prob_4band",
  ...
  if (region!=1 & region!=5) {
    ...
    vlist1<-paste0("location_value~builtuparea_pct+restrictedland_pct+localplanrate+greenbelt",
               ...
               "+I(avgtime_station>=30)",
               "+I((avgtime_station<30)*log(1+avgtime_station))",
               "+I((avgtime_station<30)*log(1+avgtime_station)^2)",
               "+I((avgtime_town>=30))",
               "+I((avgtime_town<30)*log(1+avgtime_town))",
               "+I((avgtime_town<30)*log(1+avgtime_town)^2)")
  } else if (region==1) {
    # CAMKOX
    vlist1<-paste0("location_value~builtuparea_pct+restrictedland_pct+localplanrate+greenbelt",
                   "+lu_domestic_shr+lu_gardens_shr+lu_nondom_shr+lu_road_shr+lu_rail_shr",
