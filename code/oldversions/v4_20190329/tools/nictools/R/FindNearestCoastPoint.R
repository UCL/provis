...
#' newdata<-FindNearestCoastPoint(houses,region_polygon,region_id,dirs)
FindNearestCoastPoint<-function(houses,region_polygon,region_id,dirs,basis="glp") {
  ...
  i1<-gIntersects(coast,region_polygon,byid=TRUE)
  ...
  traveltime<-predictTravelTime2(newdata,destname = "temp",region_id,dirs$traveldir,
                                 printstr="coast",basis)

  # put results in dataframe and give names to each column
  newdata<-cbind(newdata[,3:4],mindist/1000,traveltime$drive_temp,traveltime$trans_temp)
