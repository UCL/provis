...
if (depvar=="logvalue") {
  ...
  load(paste0(dirs$outdir,"/m1",depvar,"0.RData"))
  lambda <- m1boxcoxvalue0$lambda
  newprice <- (1+lambda*out1$new)^(1/lambda)
  oldprice <- (1+lambda*out0$new)^(1/lambda)
}
...
\title{Region:   \Sexpr{regnames$name[region_id]} \\
       ...
       Impact of \Sexpr{infrastructure_type} improvement project near \Sexpr{project_location} }
