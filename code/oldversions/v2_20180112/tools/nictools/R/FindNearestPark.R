...
#' newdata<-FindNearestPark(houses,parktype="AONB",region_id,dirs)
#' newdata<-FindNearestPark(houses,parktype="natpark",region_id,dirs)
FindNearestPark<-function(houses,parktype="AONB",region_id,dirs) {
  ...
  i1<-gIntersects(park,houses_convexhull,byid=TRUE)
  ...
  if (any(i1)) {
    ...
    for (i2 in 1:n1) {
      ...
      temp<-gIntersects(houses,park[i2,],byid=TRUE)
