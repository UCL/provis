...
ui <- fluidPage(
  sidebarPanel(
    ...
    conditionalPanel(
      ...
      tags$p("EXPERIMENTAL VERSION: email questions/comments to l.nesheim@ucl.ac.uk",style="color:red"),
    ...
    conditionalPanel(
      ...
      # CHOOSE ROAD
      #    INPUTS:   (method,roadfile,speed,speedfile,multiplier,run_transport)
      conditionalPanel(
      ...
      # CHOOSE RAIL
      #    INPUTS:   (method2,railfile,speed,speedfile,multiplier,run_rail)
      conditionalPanel(
        ...
        conditionalPanel(
          ...
          fileInput("speedfile",label="Choose file containing rail travel improvement area."),
...
server <- function(input, output) {
  ...
  # id:      project id 
  # dir:     location to store results
  # m2flag:  model to use for model 2 predictions (0,1, or 5)
  project <- reactiveValues(id=1,dir="www/pid1",m2flag=1)
  ...
  observeEvent(input$run_transport,{
    ...
    names(parms)<-c("project_id","region_id","infrastructure_type","method",
                    "roadfile","speedfile","speed","multiplier","m2flag")
  ...
  observeEvent(input$run_rail,{
    ...
    parms<-list(project$id,region$id,input$infrastructure_type,input$method2,
                shapefile$rail,shapefile$speed,input$speed,input$multiplier,project$m2flag)
    names(parms)<-c("project_id","region_id","infrastructure_type","method",
                    "railfile","speedfile","speed","multiplier","m2flag")
    save(parms,file=paste0(project$dir,"/parms.RData"))
  ...
  output$p2_map_text<-renderText({
    paste0("Below is a map of the ",region$name," region. Choose investment characteristics at ",
